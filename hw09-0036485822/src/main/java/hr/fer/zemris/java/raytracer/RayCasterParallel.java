...
public class RayCasterParallel {
  ...
  public static class Calculation extends RecursiveAction {
     ...
     * @param xAxis x axis
     * @param yAxis y axis
     * @param horizontal horizontal size of window
     * @param vertical vertical size of window
     * @param width image width
     * @param height image height
     * @param screenCorner screen corner
     * @param scene scene
     * @param rgb color storage
     * @param eye point from which the scene is observed
     * @param red buffer for red color
     * @param green buffer for green color
     * @param blue buffer for blue color
     * @param yMin row from which this instance will draw
     * @param yMax row to which this instance will draw
     */
    public Calculation(Point3D xAxis, Point3D yAxis, double horizontal, double vertical, int width, int height,
        Point3D screenCorner, Scene scene, short[] rgb, Point3D eye, short[] red, short[] green, short[] blue, int yMin,
        int yMax) {
      super();
      this.xAxis = xAxis;
      this.yAxis = yAxis;
    ...
    public void compute() {
      if(yMax - yMin < TRESHOLD) {
        ...
        invokeAll(
            new Calculation(xAxis, yAxis, horizontal, vertical, width, height, screenCorner, scene, rgb, eye, red,
                green, blue, yMin, yMin + (yMax - yMin) / 2 + 1),
            new Calculation(xAxis, yAxis, horizontal, vertical, width, height, screenCorner, scene, rgb, eye, red,
                green, blue, yMin + (yMax - yMin) / 2 + 1, yMax));
      }
    }

    /**
     * Method computes colors and storesthem into buffers.
     */
  ...
  private static IRayTracerProducer getIRayTracerProducer() {
    return new IRayTracerProducer() {
      ...
      public void produce(Point3D eye, Point3D view, Point3D viewUp, double horizontal, double vertical, int width,
        ...
        pool.invoke(new Calculation(xAxis, yAxis, horizontal, vertical, width, height, screenCorner, scene, rgb, eye,
